...
# from pd_netsuite.constants import (
    ...
    for idx, field_id in enumerate(file_fields, start=1):
        try:
            val = next(
                (
                    f["value"]
                    for f in document_data.get("fields", [])
...
if pandadoc_doc_id:
    with st.spinner("Fetching PandaDoc document details..."):
        ...
        pandadoc_url = (
            f"https://api.pandadoc.com/public/v1/documents/{pandadoc_doc_id}/details"
        )
    ...
    CUSTOM_FORM_ENUM = {
        ...
        "PandaDoc Poland - New Vendor Request Form": "48",
        "PandaDoc Portugal - New Vendor Request Form": "71",
        "PandaDoc Ukraine - New Vendor Request Form": "72",
    }
    CATEGORY_ENUM = {
        "Contractor": "1",
        "IT Services": "2",
    ...
    PANDADOC_TO_NETSUITE_FIELD_IDS = {
        ...
        "Checkbox2": "custrecord1533",
        "Checkbox3": "custrecord_complete_nda",
        "Checkbox4": "custrecord_vr_confirm",
        "Checkbox5": "custrecord1534",
        "Checkbox6": "custrecord1535",
        "Checkbox7": "custrecord_vr_duplicate",
    }
    # Field mapping (as in prior code)
    field_id_to_value = {
        field["field_id"]: field["value"] for field in pandadoc_data.get("fields", [])
    }
        ...
        preview_rows.append(
            {
                ...
                "Payload Value": (
                    display_value if display_value != None else "❌ Missing"
                ),
            }
        )
    ...
    if "custrecord_vr_website" in netsuite_payload:
        netsuite_payload["custrecord_vr_website"] = fix_url(
            netsuite_payload["custrecord_vr_website"]
        )
    ...
    files_payload = collect_files_from_pandadoc(
        pandadoc_data, ["CollectFile1", "CollectFile2"]
    )
    ...
    try:
        ...
        if result.get("recordId"):
            ...
            st.markdown(
                f"[Open NetSuite Record]({netsuite_url})", unsafe_allow_html=True
            )
            st.balloons()
        else:
